    ...
    positionColumns = function(){
	    ...
	    $("#cf-table .cf-column").each(function(index){
		    ...
		    if((index+1) == $('.cf-column').length){
			    ...
			    $('.cf-column.cf-middle.personal-data').height((columnHeight+40) + 'px');
		    }
	    });
	    // Results
...
function startTour(){
              introJs().setOptions({"nextLabel": " > ", "prevLabel": " < ", "skipLabel": "stop", "doneLabel": "done"}).start();
}
...
app.controller('optionsController', function($scope,$modal){
  ...
  $scope.total_cost = function(product) {
    if (!product) {
      return;
    }
    if ($scope.options.domicieleringOptional) {
      return parseFloat(product.total_cost.value);
    } else {
      return parseFloat(product.total_cost_no_dom.value);
    }
  }
  ...
  $scope.doSomeAccordionStuff = function (){
	...
	var updateArrays = function(section,obj){
		// Domiciliering
		var domicielering = ($scope.options.domicieleringOptional ? true : (obj.dom_required == 'True' ? false : true ));
		// Min-score
	...
	$scope.updateResults = function(){
        ...
        if (!$scope.options.domicieleringOptional && obj["korting_in_prokWh_domonly"]) {
          obj.total_discount = (parseFloat(obj.total_discount) - parseFloat(obj.korting_in_prokWh.value)).toFixed(2);
          delete obj.korting_in_prokWh;
          delete obj.korting_in_prokWh_domonly;
        }
			});
